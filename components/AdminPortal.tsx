...
export const AdminPortal: React.FC<AdminPortalProps> = ({ onLogout }) => {
  ...
  const loadData = async () => {
    ...
    const profName = await DataService.getProfessionalName();
  ...
  const handleProfessionalNameSave = async (e: React.FormEvent) => {
      e.preventDefault();
      await DataService.saveProfessionalName(professionalName);
  ...
  const handlePatientSelect = async (patientId: string, name: string) => {
    ...
    setAiSummary('');
    setIsProfileExpanded(true); // Always expand when opening a profile
  ...
  const handleProfileSave = async (e: React.FormEvent) => {
    ...
    if (patientProfile) {
      ...
      setIsProfileExpanded(false); // Minimize form on save
    }
  };
  ...
  const renderPatients = () => {
    ...
    if (selectedPatientId && patientProfile) {
      const patientData = uniquePatients.find(p => p.id === selectedPatientId) || {
        // Fallback for when filtering hides the currently selected patient
